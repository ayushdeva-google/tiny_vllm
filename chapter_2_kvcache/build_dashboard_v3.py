#!/usr/bin/env python3
"""Generate the v3 KV-cache comparison dashboard.

Same source and layout as build_redesigned_dashboard.py (v2), but the per-step
"Comparative" view is an all-operation speedup heatmap instead of just Attn/Total:
every operation that scales with sequence length (projections, MLP up/down, RoPE,
norms) shows its per-step speedup, color-graded so the big wins stand out.

Reads profile_results/profile_dashboard.html and writes:
  - profile_results/profile_dashboard_v3.html
  - presentations/chapter2-kvcache-dashboard-v3.html   (committable copy)

v2 and the original dashboard are left untouched.

    python build_dashboard_v3.py
"""
import os
import re
import json
...
OUT = os.path.join(HERE, "profile_results", "profile_dashboard_v3.html")
# A committable copy lives alongside the other presentations.
PRESENTATION = os.path.join(os.path.dirname(HERE), "presentations", "chapter2-kvcache-dashboard-v3.html")
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* heatmap */
  #heat-table td.op{text-align:left;color:var(--text);font-weight:600;font-size:11.5px}
  #heat-table td.op small{color:var(--faint);font-weight:500;display:block;font-size:10px}
  .hc{position:relative;border-radius:6px;padding:6px 4px;font-weight:700;font-size:11.5px;cursor:default;transition:.1s}
  .hc:hover{outline:1px solid rgba(255,255,255,0.25)}
  #heat-table td{padding:4px 5px}
  #heat-table th{font-size:9.5px}
  .heat-key{display:flex;align-items:center;gap:10px;margin:2px 0 12px;font-size:11.5px;color:var(--muted);flex-wrap:wrap}
  .heat-grad{height:10px;width:180px;border-radius:5px;
    background:linear-gradient(90deg,#3a2330,rgba(242,120,154,0.35) 12%,#232a38 30%,rgba(77,214,193,0.25) 45%,rgba(77,214,193,0.6) 70%,rgba(77,214,193,0.95))}
  .tip{position:fixed;z-index:60;pointer-events:none;background:#0f1626;border:1px solid var(--border);
    border-radius:8px;padding:8px 10px;font-size:11.5px;color:var(--text);box-shadow:0 12px 30px -12px #000;display:none;max-width:230px}
  .tip b{color:var(--kv)} .tip .base{color:var(--base)}
...
<div class="wrap">
  ...
  <section id="perstep">
    <div class="s-head"><div>
      ...
      <p>Every sampled checkpoint, all operations in milliseconds. The comparative view shows the per-operation speedup — not just attention. Anything that scaled with sequence length (projections, MLP up/down, RoPE, norms) speeds up too; per-token ops (Q/K/V, LM head) stay ~1×.</p>
    </div></div>
    <div class="tabs">
      <button class="on" data-tab="delta" onclick="showTab('delta')">Δ Comparative (all ops)</button>
    ...
    <div id="heat-wrap">
      <div class="heat-key">
        <span>speedup =&nbsp;naive&nbsp;/&nbsp;cached&nbsp;(×)</span>
        <span class="heat-grad"></span>
        <span>slower&nbsp;·&nbsp;1×&nbsp;·&nbsp;faster →</span>
        <span style="color:var(--faint)">hover a cell for raw ms · rows sorted by peak speedup</span>
      </div>
      <div class="t-wrap"><table id="heat-table" class="mono"></table></div>
    </div>
    <div id="raw-wrap" style="display:none"><div class="t-wrap"><table id="perstep-table" class="mono"></table></div></div>
  </section>
  ...
  <div class="foot">profile_dashboard_v3.html · all-operation speedup view · generated by build_dashboard_v3.py from chapter-2 rerun data</div>
</div>
...
/* ---------- comparative heatmap (all operations) ---------- */
function heatColor(sp){
  // sp<1 slower -> pink; ~1 neutral; >1 faster -> teal, log-scaled
  if(sp < 0.95){
    const a=Math.min(0.4,(0.95-sp))+0.12;
    return {bg:`rgba(242,120,154,${a.toFixed(2)})`, fg:'#ffd7e2'};
  }
  if(sp < 1.15) return {bg:'rgba(136,149,172,0.10)', fg:'var(--muted)'};
  const f=Math.min(1, Math.log10(sp)/Math.log10(250)); // 1x..250x -> 0..1
  const a=(0.14 + f*0.78).toFixed(2);
  return {bg:`rgba(77,214,193,${a})`, fg: f>0.5?'#04140f':'#d7fff6'};
}
function fmtx(sp){
  if(sp>=100) return Math.round(sp)+'×';
  if(sp>=10)  return sp.toFixed(0)+'×';
  return sp.toFixed(1)+'×';
}
function renderHeatmap(){
  const steps=DATA.steps, c2=DATA.ch2_cols, c1=DATA.ch1_cols;
  const idx2=Object.fromEntries(c2.map((c,i)=>[c,i+1]));
  // build rows for every comparable op (present in baseline) + a Total row
  const RES=0.1; // ms: naive time below this is at timer-noise level -> ratio not meaningful
  let rows = c1.map(op=>{
    const j=idx2[op];
    const sps = steps.map((s,i)=>{
      const b=DATA.ch1[i][c1.indexOf(op)+1], k=DATA.ch2[i][j];
      return {sp: (k>0? b/k : 1), b, k, ok: b>=RES};
    });
    const meaningful=sps.filter(x=>x.ok).map(x=>x.sp);
    const peak = meaningful.length ? Math.max(...meaningful) : null;
    const tiny = peak===null; // never rises above timer resolution
    return {op, sps, peak, tiny};
  });
  // Total (whole step) row from total_base/total_kv
  const totalRow={op:'Total step', sps:steps.map((s,i)=>{
    const b=DATA.total_base[i], k=DATA.total_kv[i]; return {sp:(k>0?b/k:1),b,k};
  }), peak:Math.max(...steps.map((s,i)=>DATA.total_base[i]/DATA.total_kv[i])), tiny:false, total:true};
  // sort operations by peak speedup (desc); noise-only ops fall to the bottom
  rows.sort((a,b)=>(b.peak||0)-(a.peak||0));
  rows.push(totalRow);

  let h='<thead><tr><th>Operation</th>'+steps.map(s=>`<th>#${s}</th>`).join('')+'<th>Peak</th></tr></thead>';
  let body='<tbody>';
  rows.forEach(r=>{
    const opLabel = r.total
      ? `<td class="op" style="color:#fff">${r.op}<small>full forward pass</small></td>`
      : `<td class="op">${r.op}${r.tiny?'<small>below timer resolution</small>':''}</td>`;
    let cells='';
    r.sps.forEach((x,i)=>{
      const show = r.total || x.ok;                     // hide noise cells (naive < 0.1 ms)
      const col = show ? heatColor(x.sp) : {bg:'transparent', fg:'var(--faint)'};
      const tip=`${r.op} · step #${steps[i]}||<span class='base'>naive ${x.b.toFixed(x.b<1?3:2)} ms</span> → <b>cached ${x.k.toFixed(x.k<1?3:2)} ms</b>||${show?fmtx(x.sp)+' faster':'below timer resolution'}`;
      cells+=`<td><div class="hc" style="background:${col.bg};color:${col.fg}" data-tip="${tip}">${show?fmtx(x.sp):'·'}</div></td>`;
    });
    const pk = r.peak!==null ? heatColor(r.peak) : {bg:'transparent', fg:'var(--faint)'};
    cells+=`<td><div class="hc" style="background:${pk.bg};color:${pk.fg};font-weight:800">${r.peak!==null?fmtx(r.peak):'·'}</div></td>`;
    body+=`<tr>${opLabel}${cells}</tr>`;
  });
  body+='</tbody>';
  document.getElementById('heat-table').innerHTML=h+body;
  wireTips();
}
function wireTips(){
  const tip=document.getElementById('tip');
  document.querySelectorAll('#heat-table .hc').forEach(el=>{
    el.onmousemove=e=>{
      const parts=el.dataset.tip.split('||');
      tip.innerHTML=`<div style="color:var(--faint);margin-bottom:3px">${parts[0]}</div><div>${parts[1]}</div><div style="margin-top:3px;font-weight:700">${parts[2]}</div>`;
...
/* ---------- raw per-step tables (with/without KV) ---------- */
...
function showTab(mode){
  ...
  document.getElementById('heat-wrap').style.display = delta?'block':'none';
  document.getElementById('raw-wrap').style.display = delta?'none':'block';
  if(delta) renderHeatmap(); else renderPerstep(mode);
}
renderHeatmap();
